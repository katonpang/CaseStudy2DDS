...
Notably, 70% of employees who left the company were at the lowest stock option level. This underscores stock option level as one of the top influential factors in employee attrition.  
...
Notably, 50% of those who left the company were single, underscoring marital status as one of the top influential factors in employee attrition.  
...
Job role is one of the top influential factors in employee attrition.   
...
2. The top influential factors in employee attrition are:  
    - Stock Option Level   
    - Age/Generation   
    - Marital Status   
    - Total Working Years  
...
We conducted numerous iterations of Naive Bayes and k-Nearest Neighbors (k-NN) models, experimenting with various combinations and quantities of variables. They were evaluated based on whether they met the criteria, as well as accuracy and F1 score.  

The final combination of variables chosen includes JobInvolvement, StockOptionLevel, OverTime, MaritalStatus, and JobRole. This combination was chosen because it has relatively higher sensitivity, specificity, and F1 score, with the least amount of variables used to avoid overfitting.   
...
Accuracy, Sensitivity, Precision, Recall, and F1 from the k-NN model are higher than those from the Naive Bayes model, all surpassing 70%.   
...
Monthly Income appears to be correlated with TotalWorkingYears, as indicated by the scatterplot.  
...
2. Model Comparison:
   ...
   - k-NN outperformed Naive Bayes in terms of accuracy, sensitivity, precision, recall,      and F1 score, each exceeding 70%.  
